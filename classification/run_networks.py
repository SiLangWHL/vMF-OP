...
from utils_multi_K_vMF import multi_get_loss

class model ():
    def __init__(self, config, data, test=False):
        self.config = config
        self.training_opt = self.config['training_opt']
        self.op_opt = self.config['overlap_opt']
        self.data = data
        self.test_mode = test
...
##
            ...
            if 'classifier' in key:
                ic = 1
                for key_c, val_c in self.networks[key].named_parameters():
                    if 'Kappa' in key_c:
                        decay = optim_params['kappa_decay']
                        #lr = 0.1
                    else:
                        decay = optim_params['weight_decay']
                    lr = optim_params['lr']
                    self.model_optim_params_dict[key+str(ic)] = {'params': val_c,
                                                'lr': lr,
            ...
            if 'optimizer' in self.training_opt and self.training_opt['optimizer'] == 'adam':
                ...
                if key == 'classifier':
                    if 'vmf' in self.config['networks']['classifier']['def_file']:
                        optimizer = optim.Adam([optim_params_dict[key+str(1)], optim_params_dict[key+str(2)]])
                    else:
                        optimizer = optim.Adam([optim_params_dict[key+str(1)], ])
                else:
                ...
                if key == 'classifier':
                    if 'vmf' in self.config['networks']['classifier']['def_file']:
                        optimizer = optim.SGD([optim_params_dict[key+str(1)], optim_params_dict[key+str(2)]])
                    else:
                        optimizer = optim.SGD([optim_params_dict[key+str(1)], ])
                else:
        ...
        if not feature_ext:
            if phase == 'train':
                if 'vmf' in self.config['networks']['classifier']['def_file']:
                    ...
                    var_dic = self.networks['classifier'].module._get_all_var()
    ...
    def train(self):
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                self.batch, self.group, rgb, hei, wid = inputs.size()
                #print(inputs.size())
                inputs = inputs.reshape(self.batch * self.group, rgb, hei, wid).cuda()
                labels = labels.unsqueeze(1).repeat(1, self.group).reshape(-1).cuda()
                ...
                with torch.set_grad_enabled(True):
                    ...
                    if step % self.training_opt['display_step'] == 0:
                        ...
                        if 'icd' in self.op_opt['auxloss']:
                            loss_info['icd'] = minibatch_loss_icd
    ...
    def load_model(self, model_dir=None):
        ...
        if 'CIFAR' in self.training_opt['dataset']:
            ...
            #model_state = checkpoint['state_dict'] ## 简单修改
