...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, phase, imbalance_ratio, root = '/gruntdata5/kaihua/datasets', imb_type='exp'):
        ...
        if self.train:
            ...
            self.transform = transforms.Compose([transforms.RandomCrop(32, padding=4, fill=128), # fill parameter needs torchvision installed from source
...
class IMBALANCECIFAR100(torchvision.datasets.CIFAR100):
    ...
    def __init__(self, phase, imbalance_ratio, root = '/home/Public/wanghualiang/fsm/cifar100', img_num=2, imb_type='exp'):
        train = True if phase == "train" else False
        super(IMBALANCECIFAR100, self).__init__(root, train, transform=None, target_transform=None, download=False)
        self.train = train
        ...
        if self.transform is not None:
            if self.train:
                ...
                for i in range(self.img_num):
                    ...
                    img_lis.append( self.preprocess( img_i ).unsqueeze(0) )
                    #sample_lis.append( aug(sample_i, self.preprocess).unsqueeze(0) ) 
                img = torch.cat(img_lis, 0)
            else:
                img = self.transform(img)
            #img = self.transform(img)
